...
func (s *MemoryTestSuite) TestAdd() {
	s.Nil(s.memory.Put("name", "Goframework", 1*time.Second))
...
func (s *MemoryTestSuite) TestForever() {
	s.True(s.memory.Forever("name", "Goframework"))
	s.Equal("Goframework", s.memory.Get("name", "").(string))
	s.True(s.memory.Flush())
}
...
func (s *MemoryTestSuite) TestForget() {
	...
	err := s.memory.Put("test-forget", "goframework", 5*time.Second)
	s.Nil(err)
	s.True(s.memory.Forget("test-forget"))
}

func (s *MemoryTestSuite) TestFlush() {
	s.Nil(s.memory.Put("test-flush", "goframework", 5*time.Second))
	s.Equal("goframework", s.memory.Get("test-flush", nil).(string))
...
func (s *MemoryTestSuite) TestHas() {
	...
	s.Nil(s.memory.Put("test-has", "goframework", 5*time.Second))
...
func (s *MemoryTestSuite) TestPull() {
	...
	s.Equal("Goframework", s.memory.Pull("name", "").(string))
...
func getMemoryStore() (*Memory, error) {
	mockConfig := &configmock.Config{}
	mockConfig.On("GetString", "cache.prefix").Return("goframework_cache").Once()
