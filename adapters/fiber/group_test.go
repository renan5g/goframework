...
func TestGroup(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			url:            "/middlewares/1",
			expectCode:     http.StatusOK,
			expectBodyJson: "{\"ctx\":\"Goframework\",\"ctx1\":\"Hello\",\"id\":\"1\"}",
		},
		{
		...
		},
		{
			...
			expectCode:     http.StatusOK,
			expectBodyJson: "{\"ctx\":\"Goframework\",\"ctx2\":\"World\",\"id\":\"1\"}",
		},
		{
			...
			setup: func(req *http.Request) {
				...
				fiber.GlobalMiddleware(func(ctx contractshttp.Context) {
					ctx.WithValue("global", "goframework")
					ctx.Request().Next()
				})
			...
			expectCode:     http.StatusOK,
			expectBodyJson: "{\"global\":\"goframework\"}",
		},
		{
