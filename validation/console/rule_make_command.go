...
func (receiver *RuleMakeCommand) populateStub(stub string, name string) string {
	...
	stub = strings.ReplaceAll(stub, "dummy_package", str.Camel2Case(packageName))
...
func (receiver *RuleMakeCommand) getPath(name string) string {
	...
	return filepath.Join(pwd, "internal", "application", "rules", str.Camel2Case(folderPath), str.Camel2Case(ruleName)+".go")
}
