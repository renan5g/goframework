...
import (
	...
	contractshttp "github.com/renan5g/goframework/contracts/http"
	"github.com/renan5g/goframework/contracts/validation"
	configmock "github.com/renan5g/goframework/mocks/config"
	"github.com/renan5g/goframework/support/carbon"
	"github.com/renan5g/goframework/support/file"
)

var singleLog = "storage/logs/goframework.log"
var dailyLog = fmt.Sprintf("storage/logs/goframework-%s.log", carbon.Now().ToDateString())
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Debug("Debug Goframework")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.debug: Debug Goframework"))
				assert.True(t, file.Contain(dailyLog, "test.debug: Debug Goframework"))
			},
		},
		{
			name: "No Debug",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Debug("No Debug Goframework")
			},
			assert: func() {
				assert.False(t, file.Contain(singleLog, "test.debug: No Debug Goframework"))
				assert.False(t, file.Contain(dailyLog, "test.debug: No Debug Goframework"))
			},
		},
		{
			name: "Debugf",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Debugf("Goframework: %s", "World")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.debug: Goframework: World"))
				assert.True(t, file.Contain(dailyLog, "test.debug: Goframework: World"))
			},
		},
		{
			name: "Info",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Info("Goframework")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.info: Goframework"))
				assert.True(t, file.Contain(dailyLog, "test.info: Goframework"))
			},
		},
		{
			name: "Infof",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Infof("Goframework: %s", "World")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.info: Goframework: World"))
				assert.True(t, file.Contain(dailyLog, "test.info: Goframework: World"))
			},
		},
		{
			name: "Warning",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Warning("Goframework")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.warning: Goframework"))
				assert.True(t, file.Contain(dailyLog, "test.warning: Goframework"))
			},
		},
		{
			name: "Warningf",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Warningf("Goframework: %s", "World")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.warning: Goframework: World"))
				assert.True(t, file.Contain(dailyLog, "test.warning: Goframework: World"))
			},
		},
		{
			name: "Error",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Error("Goframework")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.error: Goframework"))
				assert.True(t, file.Contain(dailyLog, "test.error: Goframework"))
			},
		},
		{
			name: "Errorf",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Errorf("Goframework: %s", "World")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.error: Goframework: World"))
				assert.True(t, file.Contain(dailyLog, "test.error: Goframework: World"))
			},
		},
		{
			name: "Panic",
			...
			assert: func() {
				assert.Panics(t, func() {
					log.Panic("Goframework")
				})
				assert.True(t, file.Contain(singleLog, "test.panic: Goframework"))
				assert.True(t, file.Contain(dailyLog, "test.panic: Goframework"))
			},
		},
		{
			name: "Panicf",
			...
			assert: func() {
				assert.Panics(t, func() {
					log.Panicf("Goframework: %s", "World")
				})
				assert.True(t, file.Contain(singleLog, "test.panic: Goframework: World"))
				assert.True(t, file.Contain(dailyLog, "test.panic: Goframework: World"))
			},
		},
		{
			name: "Code",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Code("code").Info("Goframework")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.info: Goframework\ncode: \"code\""))
				assert.True(t, file.Contain(dailyLog, "test.info: Goframework\ncode: \"code\""))
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Hint("hint").Info("Goframework")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.info: Goframework\nhint: \"hint\""))
				assert.True(t, file.Contain(dailyLog, "test.info: Goframework\nhint: \"hint\""))
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.In("domain").Info("Goframework")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.info: Goframework\ndomain: \"domain\""))
				assert.True(t, file.Contain(dailyLog, "test.info: Goframework\ndomain: \"domain\""))
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Owner("team@goravel.dev").Info("Goframework")
			},
			assert: func() {
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Request(&TestRequest{}).Info("Goframework")
			},
			assert: func() {
				expectedParts := []string{
					`test.info: Goframework`,
					`request: {`,
					`"method":"GET`,
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Response(&TestResponse{}).Info("Goframework")
			},
			assert: func() {
				expectedParts := []string{
					`test.info: Goframework`,
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.User(map[string]any{"name": "kkumar-gcc"}).Info("Goframework")
			},
			assert: func() {
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.With(map[string]any{"key": "value"}).Info("Goframework")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.info: Goframework\ncontext: {\"key\":\"value\"}"))
				assert.True(t, file.Contain(dailyLog, "test.info: Goframework\ncontext: {\"key\":\"value\"}"))
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.WithTrace().Info("Goframework")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.info: Goframework\ntrace:"))
				assert.True(t, file.Contain(dailyLog, "test.info: Goframework\ntrace:"))
...
func TestLogrus_Fatal(t *testing.T) {
	...
	assert.True(t, file.Contain(singleLog, "test.fatal: Goframework"))
	assert.True(t, file.Contain(dailyLog, "test.fatal: Goframework"))
