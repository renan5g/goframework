	...
	tests := []struct {
		...
		},
		{
			...
			assert: func() {
				...
				assert.True(t, file.Contain("packages/sms/facades/sms.go", "renan5g/gopackages/sms"))
				assert.True(t, file.Contain("packages/sms/facades/sms.go", "renan5g/gopackages/sms/contracts"))
				assert.Nil(t, file.Remove("packages"))
			},
		},
		{
			name: "name is github.com/renan5g/gosms and use other root",
			setup: func() {
				mockContext.On("Argument", 0).Return("github.com/renan5g/gosms-aws").Once()
