...
func (s *ContainerTestSuite) TestBind() {
	callback := func(app core.Application) (any, error) {
		return 1, nil
	}
	...
	case func(app core.Application) (any, error):
...
func (s *ContainerTestSuite) TestBindWith() {
	callback := func(app core.Application, parameters map[string]any) (any, error) {
	...
	s.NotNil(ins.concrete)
	switch concrete := ins.concrete.(type) {
	case func(app core.Application, parameters map[string]any) (any, error):
...
func (s *ContainerTestSuite) TestMake() {
	tests := []struct {
		...
		},
		{
			...
			key:  "Singleton",
			setup: func() {
				s.container.Singleton("Singleton", func(app core.Application) (any, error) {
					return 1, nil
				})
			},
			expectImpl: 1,
		},
		{
			...
			key:  "Bind",
			setup: func() {
				s.container.Bind("Bind", func(app core.Application) (any, error) {
					return 1, nil
				})
			},
			expectImpl: 1,
		},
		{
			...
			setup: func() {
				s.container.BindWith("BindWith", func(app core.Application, parameters map[string]any) (any, error) {
					return parameters["name"], nil
				})
			},
			expectImpl: "goravel",
